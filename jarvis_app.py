...
def backend_main(ui_q: mp.Queue, _cmd_q: mp.Queue):
    ...
    PROMPT=("너는 자비스야. 나한테 완전 친한 친구처럼 말해.\n"
            ...
            "너는 7축 로봇팔도 제어할 수 있어. 관절 이름은 joint1~joint7이야.\n"
            "로봇 제어 명령이 들어오면 자연스러운 대답 뒤에 반드시 아래 형식으로 명령을 붙여:\n"
            "[CMD:{\"type\":\"set_joints\",\"angles\":{\"joint1\":라디안값}}]\n"
            "각도는 반드시 라디안으로 변환해서 넣어. 관절 리밋:\n"
            "joint1: -1.57~1.57, joint2: -0.17~1.57, joint3: -1.57~1.57,\n"
            "joint4: -0.87~1.57, joint5~7: -1.57~1.57\n"
            "홈/원점 명령이면 [CMD:{\"type\":\"home\"}], "
            "켜기는 [CMD:{\"type\":\"enable_all\"}], "
            "끄기는 [CMD:{\"type\":\"disable_all\"}]")
    ...
    from robot_intent import parse_voice, parse_llm, execute as robot_exec
    robot = None
    try:
        from joint_controller import JointController
        robot = JointController()
        send({"t":"status","v":"ROBOT_READY"})
    except Exception as _re:
        send({"t":"status","v":f"ROBOT_OFF ({_re})"})

    now=lambda: __import__('datetime').datetime.now().strftime("%H:%M:%S")
    greeting="어 나왔어. 뭐 필요해?" if robot is None else "어 나왔어. 로봇도 연결됐어."
    ...
    while True:
        try:
            ...
            # ── 1단계: regex로 직접 명령 파싱 ─────────────────────────────
            direct_cmd = parse_voice(text)
            if direct_cmd:
                reply = robot_exec(robot, direct_cmd)
                send({"t":"ai","ts":now(),"text":reply})
                tts.put(reply); tts.done()
                hist.append({"role":"user","content":text})
                hist.append({"role":"assistant","content":reply})
                send({"t":"status","v":"WAIT"}); continue

            # ── 2단계: LLM이 의도 파악 + 선택적으로 CMD 블록 생성 ──────────
            hist.append({"role":"user","content":text})
            ...
            for ck in client.chat(model=LLM,messages=msgs,stream=True):
                ...
                if len(parts)>1:
                    for s in parts[:-1]:
                        if s.strip():
                            speech,_=parse_llm(s)   # CMD 블록 제거 후 TTS
                            if speech.strip():
                                tts.put(speech.strip())
                                if first: send({"t":"status","v":"TTS"}); first=False
                    sbuf=parts[-1]
            if sbuf.strip():
                speech,_=parse_llm(sbuf)
                if speech.strip(): tts.put(speech.strip())

            # LLM 전체 응답에서 CMD 블록 추출 후 실행
            speech_full, llm_cmd = parse_llm(full)
            if llm_cmd:
                result = robot_exec(robot, llm_cmd)
                send({"t":"status","v":f"ROBOT: {result}"})

            hist.append({"role":"assistant","content":speech_full})
            send({"t":"stream","v":""}); send({"t":"ai","ts":now(),"text":speech_full})
